...
def _mint_id_token(user_id: str, client_id: str, nonce: str, scope: list[str]) -> str:
    ...
    return jwt.encode(id_token_claims, private_key, algorithm="RS256", headers={"kid": "oauth-lab-v091"})
...
def _mint_access_token(user_id: str, client_id: str, scope: list[str] | None = None, resource: str | None = None) -> tuple[str, int]:
    expires_at = datetime.now() + timedelta(seconds=ACCESS_TOKEN_TTL)
    scope = scope or []
