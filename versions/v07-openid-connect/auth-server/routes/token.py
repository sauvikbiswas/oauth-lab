...
def _mint_access_token(user_id: str, client_id: str, scope: list[str] | None = None) -> tuple[str, int]:
    expires_at = datetime.now() + timedelta(seconds=ACCESS_TOKEN_TTL)
    scope = scope or []
...
def _mint_token_pair(user_id: str, client_id: str, scope: list[str] | None = None) -> tuple[str, str, int]:
    access_token, expires_in = _mint_access_token(user_id, client_id, scope)
