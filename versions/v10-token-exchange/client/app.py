...
def create_app() -> Flask:
    ...
    def inject_lab_context():
        return {"lab_version": "v10", "lab_role": "client"}
    ...
    def logout():
        ...
        session.pop("exchange_error", None)
        return redirect("/")
    ...
    @app.route("/exchange-for-b", methods=["POST"])
    def exchange_for_b():
        access_token = session.get("access_token")
        if not access_token:
            if not refresh_access_token():
                session["exchange_error"] = "Access token is missing. Log in for Resource A first."
                return redirect(url_for("profile"))
            access_token = session["access_token"]

        agent_server = os.environ.get("AGENT_SERVER_URL", "http://localhost:25003").rstrip("/")
        try:
            resp = requests.post(
                f"{agent_server}/exchange",
                data={
                    "subject_token": access_token,
                    "resource": resource_b_indicator(),
                },
                timeout=10,
            )
        except requests.RequestException as exc:
            session["exchange_error"] = f"Agent unreachable: {exc}"
            return redirect(url_for("profile"))

        try:
            payload = resp.json()
        except requests.JSONDecodeError:
            session["exchange_error"] = f"Agent returned non-JSON (status {resp.status_code})"
            return redirect(url_for("profile"))

        if resp.status_code != 200 or payload.get("error"):
            desc = payload.get("error_description", payload.get("error", "Token exchange failed"))
            session["exchange_error"] = desc
            return redirect(url_for("profile"))

        session["exchanged_access_token"] = payload["access_token"]
        session.pop("exchange_error", None)
        return redirect(url_for("profile"))
    ...
    def profile():
        ...
        exchanged_token = session.get("exchanged_access_token")
        resource_b_obo = None
        if exchanged_token:
            resource_b_obo = _resource_api_result(
                fetch_resource_api(exchanged_token, "/api/resource-b")
            )
        ...
        return render_template(
            ...
            resource_b_obo=resource_b_obo,
            exchanged_token=exchanged_token,
            resource_a_indicator=_resource_indicator_for("resource-a"),
            resource_b_indicator=_resource_indicator_for("resource-b"),
            agent_server_url=os.environ.get("AGENT_SERVER_URL", "http://localhost:25003").rstrip("/"),
            exchange_error=session.pop("exchange_error", None),
        )
