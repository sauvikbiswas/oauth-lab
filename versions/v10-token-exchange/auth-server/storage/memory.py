...
# authorization_code -> {client_id, redirect_uri, code_challenge, user_id, expires_at, used, scope, nonce, resource?}
authorization_codes: dict = {}

# access_token -> {user_id, client_id, expires_at, scope, resource?}  (opaque Mode A only)
access_tokens: dict = {}

# refresh_token -> {user_id, client_id, scope, expires_at, resource?}
refresh_tokens: dict = {}
...
# Resource indicator allowlist: shared/resource_indicators.py (env or RESOURCE_SERVER_URL defaults)
...
# Token-exchange actors authenticate as OAuth clients (not redirect URIs).
# Register demo-agent here or in clients{} with a flag — agent calls POST /token with
# grant_type urn:ietf:params:oauth:grant-type:token-exchange.
# agent_client_id -> {client_secret, allowed_exchange_targets?}
agent_clients: dict = {
    "demo-agent": {
