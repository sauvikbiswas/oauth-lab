"""RFC 7662 token introspection — v10 scaffold on v09.

v09: returns `aud` (resource indicator) for active tokens.
...
def _authenticate_agent_client() -> tuple[bool, str | None]:
    client_id = request.form.get("client_id")
    client_secret = request.form.get("client_secret")
    if not client_id or not client_secret:
        return False, None
    registered = memory.agent_clients.get(client_id)
    if not registered or registered["client_secret"] != client_secret:
        return False, None
    return True, client_id
...
def _active_from_memory(token: str) -> dict | None:
    ...
    if token_data.get("act"): 
        payload["act"] = token_data.get("act")
    return payload
