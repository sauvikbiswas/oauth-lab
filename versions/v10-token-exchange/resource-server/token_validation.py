"""Token validation helpers — v10 scaffold on v09.

v09: optional expected_resource for audience checks on gated routes.
...
When present, includes `act` (RFC 8693 actor) for OBO tokens — surfaced via /debug/state last_validation.
...
def _validate_via_introspection(
    ...
    result: dict[str, Any] = {"user_id": sub}
    if body.get("act"):
        result["act"] = body["act"]
    return result, None
